...
- [ ] 会话管理行为对齐 ChatGPT 网页体验
  - [ ] 引入“草稿聊天态（无 session）”与“持久会话态（有 session_id）”双态模型
  - [ ] 页面初始化不自动创建会话；仅加载会话列表并恢复最近活跃会话（若存在）
  - [ ] 仅在用户发送首条消息时创建新会话（lazy create）
  - [ ] 删除当前会话后不自动新建会话：有剩余会话则切换，无剩余会话则回到草稿态
  - [ ] 刷新页面不应新增空白会话；修复“刷新即创建会话”问题
  - [ ] 支持会话重命名（前端交互 + 后端 API）
  - [ ] 新会话标题支持根据首条用户消息自动命名（并允许后续手动修改）
  - [ ] 会话列表交互从 `select + 删除按钮` 升级为列表项 + `...` 二级菜单（至少含重命名、删除）
  - [ ] 会话删除接口语义收敛：删除失败（不存在等）返回明确错误，前端校验 `success`
