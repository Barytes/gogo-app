...
- [ ] 桌面版中支持 Wiki Markdown 编辑模式
  ...
  - [ ] 设计 Markdown 编辑器与预览/渲染模式的切换关系，避免破坏当前浏览体验
  - [ ] 打通后端保存 API、路径安全校验与保存失败提示
  - [ ] 结合桌面版能力评估文件监听、外部修改检测、自动保存草稿等增强体验
  - [ ] 明确与 knowledge-base 规范的边界：是否允许任意改写、是否需要保留原始内容、是否要记录最近一次编辑结果
- [ ] 桌面版中在 Chat 输入框加入 slash 命令按钮，并支持 agent 的 slash 命令
  - [ ] 设计输入框中的 slash 命令入口：按钮、菜单、输入补全，还是三者结合
  - [ ] 明确命令来源：先支持 Pi / agent 已内置的 slash 命令，再评估是否展示动态命令列表
  - [ ] 设计选择命令后的行为：插入输入框、直接发送，还是先补参数再发送
  - [ ] 明确 slash 命令与普通消息、Enter 发送、停止按钮、草稿态之间的交互优先级
  - [ ] 结合桌面版 Pi CLI / 原生命令桥评估哪些 slash 命令应直接桥接，哪些只做聊天输入辅助
  - [ ] 评估是否需要在前端做命令说明、禁用态和错误提示，避免用户不知道哪些命令在当前模式下可用
